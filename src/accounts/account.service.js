...
const { accounts } = require("../config/database.conf");
...
AccountService.create = async (data) => {
  ...
  let checkExpense = await accounts.findAll({
    where: { user_id: data.user_id, currency: data.currency ?? "FC" },
  });
...
AccountService.updateBalance = async (amount, transaction, id) => {
  let closing = await AccountService.checkAccount(id);
  if (closing == false) {
    return {
      status: 401,
      message: "Ce compte n'existe pas dans le système ou il a été désactivé",
      data: [],
    };
  }
  ...
  if (
    ...
    transaction.toLowerCase() != "expense"
  ) {
    return { status: 401, data: [], message: "Invalid transaction" };
  }
  let account = await accounts.findOne({
    where: { [Op.or]: { id: id, uuid: id } },
  });
  if (!account) {
  ...
  if (transaction.toLowerCase() == "payment") {
    ...
    if (parseFloat(account.sold) - parseFloat(amount) < 0) {
      return {
        status: 401,
        data: [],
        message: "The transaction amount is great than the account balance",
      };
    }
    sold = parseFloat(account.sold) - parseFloat(amount);
  }
...
AccountService.checkAccount = async (accountUUID) => {
  let closing = await accounts.findOne({
    where: { uuid: accountUUID, active: 1 },
  });
  if (closing) {
    return true;
  }
